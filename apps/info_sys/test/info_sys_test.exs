defmodule InfoSysTest do
  ...
  alias InfoSys.Application.Result
  ...
  test "compute/2 with backend results" do
    assert [%Result{backend: "test", text: "result"}] = InfoSys.Application.compute("result", backends: [TestBackend])
  end

  test "compute/2 with no backend results" do
    assert [] = InfoSys.Application.compute("none", backends: [TestBackend])
  end

  test "compute/2 with timeout returns no results and kills workers" do
    results = InfoSys.Application.compute("timeout", backends: [TestBackend], timeout: 10)
    assert results == []
  ...
  test "compute/2 discards backend errors" do
    assert InfoSys.Application.compute("BOOM!", backends: [TestBackend]) == []
    refute_received {:DOWN, _, _, _, _}
    refute_received :timeout
  end
end
